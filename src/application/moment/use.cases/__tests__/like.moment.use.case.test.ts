...
describe("LikeMomentUseCase", () => {
    ...
    beforeEach(() => {
        ...
        likeMomentUseCase = new LikeMomentUseCase(mockMomentRepository, mockMomentService)
    })

    describe("execute", () => {
        it("deve curtir momento com sucesso", async () => {
            // Arrange
            ...
            mockMomentService.likeMoment.mockResolvedValue(undefined)
        ...
        it("deve falhar quando momento não é encontrado", async () => {
            ...
            expect(result.liked).toBeUndefined()
            expect(mockMomentService.hasUserLikedMoment).not.toHaveBeenCalled()
            expect(mockMomentService.likeMoment).not.toHaveBeenCalled()
        })

        it("deve falhar quando usuário já curtiu o momento", async () => {
            // Arrange
            const request: LikeMomentRequest = {
                momentId: "moment_123",
                userId: "user_456",
            ...
            // Assert
            expect(result.success).toBe(false)
            expect(result.error).toBe("Usuário já curtiu este momento")
            expect(result.liked).toBeUndefined()
            expect(mockMomentService.likeMoment).not.toHaveBeenCalled()
        })
        ...
        it("deve falhar quando momento não está publicado", async () => {
            ...
            // Assert
            expect(result.success).toBe(false)
            expect(result.error).toBe("Momento não pode ser curtido no estado atual")
            expect(result.liked).toBeUndefined()
            expect(mockMomentService.hasUserLikedMoment).not.toHaveBeenCalled()
            expect(mockMomentService.likeMoment).not.toHaveBeenCalled()
        })
