...
export async function jwtEncoder({
    ...
    try {
        ...
        const user = await UserModel.findByPk(userId, {
            attributes: ["id"], // Busca apenas o ID para performance
        })
        ...
        // Criar payload
        const payload = createJwtPayload(userId, username, timezone, permissionLevel, config)
        ...
        const token = await new SignJWT(payload)
            ...
            .setIssuedAt()
            .setExpirationTime(`${config.expiresIn}s`)
