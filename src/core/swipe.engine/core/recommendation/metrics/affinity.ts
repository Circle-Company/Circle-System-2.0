...
export function calculateAffinityScore(
    ...
    try {
        ...
        if (
            factors.minSimilarityThreshold !== undefined &&
 ...
 */
function calculateEmbeddingSimilarity(userEmbedding: UserEmbedding, cluster: ClusterInfo): number {
    try {
...
export function calculateTopicSimilarity(
    ...
    try {
        ...
        // Com decayFactor < 1, queremos reduzir a similaridade
        // Usar uma função que garante que decayFactor menor = similaridade menor
        const decayedSimilarity = jaccardSimilarity * decayFactor

        // Aplicar função sigmóide para melhor distribuição dos valores
        // Usar centro em 0.3 para que valores de 0.5 sejam amplificados
        const sigmoidSimilarity = 1 / (1 + Math.exp(-4 * (decayedSimilarity - 0.3)))

        // Retornar o valor com sigmóide aplicada
        return Math.max(0, Math.min(1, sigmoidSimilarity))
    } catch (error) {
...
function calculateNetworkProximity(
    ...
    try {
        ...
        for (const interaction of recentInteractions) {
            ...
            const weightedScore =
                simulatedTopicSimilarity * Math.abs(interactionWeight) * temporalWeight
...
function calculateClusterCentrality(cluster: ClusterInfo): number {
    try {
        ...
        const topicDiversityFactor = cluster.topics ? Math.min(1, cluster.topics.length / 10) : 0.5

        // Combinar fatores
        const centrality = sizeFactor * 0.4 + densityFactor * 0.3 + topicDiversityFactor * 0.3
...
export function calculateDetailedAffinityMetrics(
    ...
    try {
        ...
        const overallAffinity = calculateAffinityScore(userEmbedding, cluster, userProfile, factors)
