...
// Definir interface local para UserEmbeddingProps
...
export class UserEmbeddingService {
    ...
    private readonly dimension: number = Params.dimensions.embedding
    ...
    constructor(dimension: number = Params.dimensions.embedding) {
        ...
        this.logger = new Logger("UserEmbeddingService", {
        ...
        this.logger.info(
            `UserEmbeddingService inicializado - dimensão: ${dimension}, usando modelo: ${this.MODEL_ID}, cache: ${this.CACHE_DIR}`,
        )
    }
    ...
    protected async loadModel(): Promise<void> {
        try {
            if (!this.embeddingPipeline) {
                // Model load attempted flag removed
    ...
    private generateFallbackEmbeddingVector(userData: UserEmbeddingProps): number[] {
        ...
        const interactionVector = Array.from(interactionTypes).map((type) =>
            this.getInteractionWeight(type as string),
        )
    ...
    async updateEmbedding(
        ...
        const updatedEmbedding = currentEmbedding.map((val, idx) => {
            return (
                val * (1 - interactionWeight) + (interactionEmbedding[idx] || 0) * interactionWeight
            )
        })
