import SignLogModel, { SecurityRisk, SignStatus } from "@/infra/models/auth/sign.logs.model"
...
import StatisticModel from "@/infra/models/user/statistic.model"
...
import { jwtEncoder } from "@/infra/jwt/encode"
...
export class AuthController {
    ...
    async signIn(signData: SignRequest) {
        ...
        const newAccessToken = await jwtEncoder({
            ...
            permissionLevel: userStatusData?.access_level || Level.USER,
        })
        if (!newAccessToken)
            throw ErrorFactory.validation(
